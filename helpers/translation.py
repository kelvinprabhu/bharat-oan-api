...
def add_hindi_terms(text: str, term_pairs=term_pairs) -> str:
    """
    Add Hindi translations in brackets for matching English terms in the text.
    Example: "Click the button" -> "Click (क्लिक) the button (बटन)"
    
    Args:
        text: English text to process
        term_pairs: List of dictionaries with 'en' and 'hi' keys for term pairs
        
    Returns:
        Text with Hindi translations added in brackets for matching terms
    """
    if not text or not term_pairs:
        return text

    # Extract valid English-Hindi pairs and sort by length
    valid_terms = []
    for term in term_pairs:
        en_term = term.get('en', '').strip()
        hi_term = term.get('hi', '').strip()
        
        if not en_term or not hi_term:
            continue
            
        # Normalize whitespace
        en_term = ' '.join(en_term.split())
        hi_term = ' '.join(hi_term.split())
        valid_terms.append((en_term, hi_term))
    ...
    pattern_parts = []
    term_mapping = {}  # English lowercase -> Hindi
    case_mapping = {}  # English lowercase -> English original case
    
    for en_term, hi_term in valid_terms:
        try:
            ...
            # Store mappings
            term_mapping[en_term.lower()] = hi_term
    ...
    def replace_match(match):
        """Replace matched English term with "term (हिंदी शब्द)" """
        term = match.group(0)
        lookup_term = term.strip().lower()
        
        hindi = term_mapping.get(lookup_term, '')
        if hindi:
            original_case = case_mapping.get(lookup_term, term)
            return f"{original_case} ({hindi})"
        return term
